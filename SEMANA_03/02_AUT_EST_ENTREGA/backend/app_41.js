...
app.post('/inserePessoa', urlencodedParser, (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	sql = "INSERT INTO pessoa (nome, contato, data_de_nascimento) VALUES ('" + req.body.nome + "', '" + req.body.contato + "', " + req.body.data_de_nascimento + ")";
	console.log(sql);
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/atualizaPessoa', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = "UPDATE pessoa SET nome='" + req.body.nome + "', contato = '" + req.body.contato + "' , data_de_nascimento ='" + req.body.data_de_nascimento + "' WHERE id_pessoa='" + req.body.id_pessoa + "'";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
